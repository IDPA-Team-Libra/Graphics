...
def intraday_alpha_vantage(symbol, timeframe):
    ...
    for d,p in parsed.items():
        ...
        data_row = [float(p['1. open']), float(p['2. high']), float(p['3. low']), float(p['4. close']), int(p['5. volume'])]
        df.loc[-1,:] = data_row
...
def update_graph(theme, interval, chart_type, urlinput):
    ...
    if chart_type == 'Candlestick':
        ...
        figure.update_layout(xaxis_rangeslider_visible = False, plot_bgcolor = theme_color,
                             paper_bgcolor = theme_color, font_color = text_color, title = 'Stock Prices for '+ company +' over Time',
                             title_x = 0.5)
        ...
        figure = {'data': line_trace,
                'layout': go.Layout(
                        colorway = [line_color, '#FF4F00', '#375CB1', '#FF7400', '#FFF400', '#FF0056'],
